(function(){
  ...
  function attachLexicon(root, dict){
    ...
    Object.keys(dict).forEach(function(k){
      ...
      var base=terms[0];
      defs[base]=def;
      terms.forEach(function(t){ entries.push({term:t,key:base}); });
    });
    ...
    Array.prototype.forEach.call(root.children,function(block){
      ...
      entries.forEach(function(entry){
        ...
        while((node=walker.nextNode())){
          ...
          var p=node.parentNode; var skip=false;
          while(p && p!==block){ if(skipTags.indexOf(p.tagName)!==-1){ skip=true; break;} p=p.parentNode; }
          if(skip) continue;
    ...
    function outside(e){ if(current && !e.target.closest('.lex-pop') && !e.target.closest('.lex')) close(); }
    ...
    root.addEventListener('focusin',function(e){ var el=e.target.closest('.lex'); if(el) open(el); });
